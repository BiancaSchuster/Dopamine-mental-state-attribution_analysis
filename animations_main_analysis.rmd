...
list.of.packages <- c("here", "Select", "broom", "tidyverse", "RColorBrewer", "ggrain", "ggmcmc", "ggthemes",
                      ...
                      "performance", "remotes", "wesanderson", "ggdist")
...
mainDat <- read_csv("main_data_v2.csv") # created on 16/01/2024, added model 1.4
...
mainDat <- mainDat%>%
  ...
  mutate(arousal = as.ordered(arousal)) # tiredness ratings at T4 - just before main task
...
### compare models
anim.contr$m1.3 <- add_criterion(anim.contr$m1.3, c("loo", "waic"))

LOO(anim.main$m1.1, anim.contr$m1.3, "loo")
model_weights(anim.main$m1.1, anim.contr$m1.3, "loo")


# model 1.4
#-------------------------------------------------------------------------------
anim.contr$m1.4 <- brm(formula = accuracy  ~ drug*arousal +
                      (1+drug||sub) + 
                      (1|animID),
                    data = mainDat, 
                    family = gaussian(),
...
# extract fixed effects
model_fixef$m1.4.drug <- fixef(anim.contr$m1.4, pars = "drug2")

### compare models
anim.contr$m1.4 <- add_criterion(anim.contr$m1.4, c("loo", "waic"))

# rerun model 1.1 with the same number of observations as model 1.4 so they can be compared using LOO
mainDat.subset <-
  mainDat %>%
  filter(arousal != is.nan(arousal))

anim.main$m1.1.subs <- brm(formula = accuracy  ~ drug +
                      (1+drug||sub) + 
                      (1|animID),
                    data = mainDat.subset, 
                    family = gaussian(),
...
anim.main$m1.1.subs <- add_criterion(anim.main$m1.1.subs, c("loo", "waic"))

LOO(anim.main$m1.1.subs, anim.contr$m1.4)
model_weights(anim.main$m1.1.subs, anim.contr$m1.4, "loo")

model_fixef$m1.3.drug <- fixef(anim.contr$m1.3, pars = "drug2")
model_fixef$m1.3.arousal <- fixef(anim.contr$m1.3, pars = "drugDay2")
model_fixef$m1.3.drugXdrugDay <- fixef(anim.contr$m1.3, pars = "drug2:drugDay2")
...
# names(mainDat)[names(mainDat) == "drug"] <- "Drug"
...
ggsave("Figure_2.tiff", device = "tiff", bg="white", height = 4.5 , width = 4.5 * 1.6, dpi = 300)
```
...
diffDat[diffDat == 'NaN'] <- NA # for some reason brms adds NaNs as factor levels - replace with NA
...
library(grid)
text_more <- textGrob("more\naccurate", gp=gpar(fontsize=10, fontface="bold"))
...
Figure_3 <- ggplot(data  = diffDat,
  ...
  labs(
    ...
    scale_color_manual(name="Condition",
                       ...
                       values = colours, na.translate = FALSE) +
...
ggsave("Figure_3.tiff", device = "tiff", bg="white", height = 4.5 , width = 4.5 * 1.6, dpi = 300)
```
...
model_weights(anim.diff$m4.0, anim.diff$m4.1, "loo")
